...
import { ScheduleSummaryResponse, ScheduleResponse } from '../dto/ScheduleDto';
...
export class NotificationService {
  ...
  async sendDeadlineReminder(schedule: Schedule | ScheduleResponse, customMessage: string = '締切が1時間以内'): Promise<void> {
    if (!schedule.deadline) return;
    
    const deadlineDate = schedule.deadline instanceof Date ? schedule.deadline : new Date(schedule.deadline);
  ...
  private async getNonRespondents(summary: ScheduleSummaryResponse): Promise<string[]> {
