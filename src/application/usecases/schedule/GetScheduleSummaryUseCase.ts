...
export class GetScheduleSummaryUseCase {
  ...
  async execute(scheduleId: string, guildId: string): Promise<GetScheduleSummaryUseCaseResult> {
    try {
      ...
      return {
        success: false,
        errors: [
          `スケジュール概要の取得に失敗しました: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ],
      };
    }
  }
  ...
  private calculateResponseCounts(
    dates: Array<{ id: string; datetime: string }>,
    responses: ResponseDto[]
  ): Record<string, { yes: number; maybe: number; no: number }> {
  ...
  private calculateStatistics(
    ...
    const partiallyAvailable = responses.filter(
      (r) =>
    ...
    const unavailable = responses.filter(
      (r) => !dates.some((date) => r.dateStatuses[date.id] === 'ok')
    ).length;
